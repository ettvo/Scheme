...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    if expressions.first == 'quote':
        print("s")
        return scheme_eval(expressions.rest, env)
    return expressions.first
    # return Pair(expressions.first.first, do_quote_form(expressions.first.rest, env))
    # ''hello >> Pair('quote', Pair('quote', Pair('hello', nil))) >> (quote hello)
    # END PROBLEM 6
