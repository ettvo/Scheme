...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # read_line('((1 2 3)') >> SyntaxError (goes off to read_tail(src))
            
            # ??? might avoid case 8 error
            #if src.current() == '(':
            #    raise SyntaxError

            if src.current() == '(':
                return Pair(scheme_read(src), read_tail(src))
                #return Pair(x, Pair(scheme_read(src), nil))
                #return Pair(x, Pair(scheme_read(src), read_tail(src)))
            elif src.current() == "'":
                src.pop_first()
            ...
            #if src.current():
            #    if x.rest is nil:
            #        return Pair(x.first, read_tail(src))
            #    else:
            #        return Pair(x, read_tail(src))
            #else:
            #    return x
            # END PROBLEM 1
    except EOFError:
